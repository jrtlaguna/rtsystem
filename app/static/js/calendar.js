...
class CalendarYvv{
	constructor(etiqueta="", diaSeleccionado="", primerDia="Lunes") {
		...
		this.textColor = "text-black"; // color de texto en la cabecera
	...
	createDaysMont(){
		...
		var emptyTag = "<div class='d-flex border flex-fill w-100 justify-content-center pt-3 pb-3 btn-info' style='color:transparent'>";
		...
		for(var i=0;i<this.cantDias;i++){
			...
			var div = $("<div class='d-flex border flex-fill w-100 justify-content-center pt-3 pb-3 btn-primary "+this.btnD+"' data-date='"+fechNow+"'>").html(i+1);
			var clas_e = this;
			var _ind = (this.cantDias+posMes)%7;

			//dia seleccionado
			if(this.diaSeleccionado==fechNow){
				div = $("<div class='current-date-selected d-flex border flex-fill w-100 justify-content-center pt-3 pb-3 btn-danger "+this.btnD+"' data-date='"+fechNow+"'>").html(i+1);
			}
			//dias resaltados o importantes
			if(this.diasResal.indexOf(i+1)!=-1){
				div = $("<div class='d-flex border flex-fill w-100 justify-content-center pt-3 pb-3 btn-primary "+this.btnD+"' data-date='"+fechNow+"' style='background: "+this.colorResal+"; color: "+this.textResalt+"; font-weight: bold;'>").html(i+1);
			}
